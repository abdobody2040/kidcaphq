...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, onNavigate }) => {
  ...
  const NavContent = () => (
      <>
        <div className="flex items-center justify-between mb-8 px-2 md:block hidden">
        ...
        {isKid && (
          <div className="mb-6 space-y-3 px-2 mt-4 hidden md:block">
            ...
            <div className="flex items-center justify-between text-orange-500 font-bold p-2 bg-orange-50 dark:bg-orange-900/20 rounded-xl relative overflow-hidden">
              ...
              {hasFreeze && (
                  <div className="absolute end-0 top-0 bottom-0 bg-blue-100 dark:bg-blue-900/30 w-8 flex items-center justify-center border-s border-blue-200 dark:border-blue-800" title="Streak Protected">
            ...
            <div className="flex items-center justify-between text-yellow-600 dark:text-yellow-400 font-bold p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl">
              <div className="flex items-center gap-2"><Coins size={20} className="fill-yellow-400 text-yellow-600 dark:text-yellow-400" /> {t('stats.bizcoins')}</div>
              <motion.span 
                 key={safeCoins}
        ...
        {/* Desktop Sidebar Footer (Logout + Theme) */}
  ...
  return (
    <div className="h-screen bg-green-50 dark:bg-gray-900 flex flex-col md:flex-row font-sans overflow-hidden transition-colors duration-200">
      ...
      {/* DESKTOP SIDEBAR */}
      <nav className="hidden md:flex w-64 bg-white dark:bg-gray-800 border-e border-gray-200 dark:border-gray-700 p-4 flex-col h-full overflow-y-auto no-scrollbar z-20 transition-colors duration-200">
        <NavContent />
      </nav>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
            <motion.div 
                ...
                className="fixed inset-0 z-[200] bg-white dark:bg-gray-800 flex flex-col p-4 md:hidden"
            >
      ...
      {/* MAIN CONTENT */}
      <main className="flex-1 p-4 md:p-8 overflow-y-auto relative scroll-smooth pt-20 pb-24 md:pb-8 md:pt-8 h-full bg-green-50 dark:bg-gray-900 transition-colors duration-200">
...
const NavItem = ({ icon, label, active, onClick }: { icon: any, label: string, active: boolean, onClick: () => void }) => (
  <button
    ...
    className={`w-full flex items-center gap-4 px-4 py-3 rounded-2xl font-extrabold text-lg transition-all border-b-4 text-start
      ${active 
        ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-500 dark:text-blue-400 border-blue-200 dark:border-blue-800' 
        : 'bg-transparent text-gray-500 dark:text-gray-400 border-transparent hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-700 dark:hover:text-gray-200'}`}
  >
    {icon}
    {label}
  </button>
